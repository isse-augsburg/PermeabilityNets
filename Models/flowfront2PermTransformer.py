...
if __name__ == "__main__":
    model_inpt = torch.randn(2, 143, 111).cuda()
    model_target = torch.randn(2, 143, 111).cuda()

    model = Bumblebee2().cuda()
    pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(pytorch_total_params)
    t = model(model_inpt)
    loss = torch.nn.MSELoss()
    l = loss(t, model_target)
    print(l)

    model_inpt = torch.randn(2, 100, 143, 111).cuda()
    model_target = torch.randn(2, 135, 103).cuda()

    model = OptimusPrime2(batch_size=2, chpkpt=None).cuda()
    pytorch_total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(pytorch_total_params)
    t = model(model_inpt)
    loss = torch.nn.MSELoss()
